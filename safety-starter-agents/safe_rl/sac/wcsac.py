...
wandb.init(project='Safexp-PointGoal1-v0', settings=wandb.Settings(_disable_stats=True), \
group=f'WCSAC-0.1', name=f'{2}', entity='hmhuy')
...
def sac(env_fn, actor_fn=mlp_actor, critic_fn=mlp_critic, var_fn=mlp_var, ac_kwargs=dict(), seed=0,
    ...
    def test_agent(n=30):
        ...
        logger.store(failure_rate=failed/n)
    ...
    for t in range(total_steps // num_procs()):
        ...
        if t > 0 and t % local_steps_per_epoch == 0:
            ...
            failure_rate_mean,failure_rate_std,_,_ = logger.log_tabular('failure_rate')
            log_info = {}
            log_info['validation/failure_rate'] = failure_rate_mean

            logger.log_tabular('EpRet', with_min_and_max=True)
            return_val_mean,return_val_std,_,_ = logger.log_tabular('TestEpRet', with_min_and_max=True)
            log_info['validation/return'] = return_val_mean
            logger.log_tabular('EpCost', with_min_and_max=True)
            cost_val_mean,cost_val_std,cost_val_max,cost_val_min = logger.log_tabular('TestEpCost', with_min_and_max=True)
            log_info['validation/cost'] = cost_val_mean
            log_info['validation/cost_std'] = cost_val_std
            log_info['validation/cost_max'] = cost_val_max
            log_info['validation/cost_min'] = cost_val_min
            ...
            logger.dump_tabular()
            Wandb_logging(diction=log_info,step_idx=valid_cnt,wandb_logs=True)
            valid_cnt += 1
